...
import { CopyIcon, LockIcon, PlusSquare, Trash2, UnlockIcon } from 'lucide-react';
import React from 'react'

export default function MobileDisplay({palette}: {
    palette: {
        rgb: string;
        isLocked: boolean;
        isBaseColor: boolean;
        ...
        {palette.map(({rgb, isLocked}) => (
           ...
           className='h-full rounded-lg flex items-center justify-center'>
            <div className='font-inter text-2xl font-bold uppercase flex items-center gap-2 justify-around w-full px-8'>
              ...
              {isLocked ? <Button className='text-black/60' size="icon" variant={"ghost"}>
              <LockIcon/>
              </Button> : <Button className='text-black/60' size="icon" variant={"ghost"}>
              <UnlockIcon/>
              </Button>}
              <Button className='text-black/60' size="icon" variant={"ghost"}>
              <Trash2/>
