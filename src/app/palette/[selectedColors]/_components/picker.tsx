...
export default function ColorPicker({ initColor, onChange }: { initColor: string, onChange: (color: string) => void }) {
  ...
  const [color, setColor] = useState<RGB>(parseRgb(initColor))
  const [hsv, setHsv] = useState<HSV>(rgbToHsv(parseRgb(initColor)))
  ...
  const handlePickerMouseDown = (e: React.MouseEvent<HTMLDivElement>) => {
    const updateColor = (clientX: number, clientY: number) => {
      ...
      console.log("updateColor", clientX, clientY)
    }
    ...
    const handleMouseUp = () => {
      ...
      router.push(`/palette/${rgbToHex(color).replace("#", "")}`)
    }
