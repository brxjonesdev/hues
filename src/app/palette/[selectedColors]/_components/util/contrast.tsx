...
export default function ContrastCheck({ color }: { color: string }) {
  ...
  return (
    <Popover open={isOpen} onOpenChange={setIsOpen}>
      ...
      <PopoverContent className="w-fit p-2 space-y-2 font-nunito" side="top" align="center">
      <div className="grid grid-cols-2 gap-2 rounded ">
              <div  className='rounded-lg text-center'
              style={{ backgroundColor: baseColor, color: 'white', padding: '0.5rem', fontSize: '0.75rem' }}>
                <span>White / </span>
                <span className="font-mono text-sm">{whiteContrast}</span>
                <span className="ml-2 text-xs">({getContrastLevel(whiteContrast)})</span>
              </div>
              <div  className='rounded-lg text-center'
              style={{ backgroundColor: baseColor, color: 'black', padding: '0.5rem', fontSize: '0.75rem' }}>
                <span>Black / </span>
                <span className="font-mono text-sm">{blackContrast}</span>
                <span className="ml-2 text-xs">({getContrastLevel(blackContrast)})</span>
              </div>
        </div>
        <div>
        <Link href={`/contrast/${color.replace('#', '')}-ffffff`} className='text-sm'>
