...
export function HexToRGB(hex: string): string {
  ...
  // Convert Hex to LAB (approximate)
  export function HexToLAB(hex: string): string {
    ...
    [r, g, b] = [r, g, b].map(v => (v > 0.04045 ? Math.pow((v + 0.055) / 1.055, 2.4) : v / 12.92) * 100);
    const x = (0.4124564 * r + 0.3575761 * g + 0.1804375 * b) / 95.047;
    const y = (0.2126729 * r + 0.7151522 * g + 0.0721750 * b) / 100.0;
    const z = (0.0193339 * r + 0.1191920 * g + 0.9503041 * b) / 108.883;
    const lab = (v: number) => (v > 0.008856 ? Math.cbrt(v) : (7.787 * v) + (16 / 116));
